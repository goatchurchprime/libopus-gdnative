...
func _on_compress_pressed():
	if recording:
		var rawpcm = recording.get_data()
		opusencoded = $OpusEncoderNode.encode(rawpcm)
		print(" Recording leng ", len(rawpcm), " encoded leng ", len(opusencoded))

func _on_play_compressed_pressed():
	if opusencoded:
		var decoded = $OpusDecoderNode.decode(opusencoded)
		print(" Recording decoded leng ", len(decoded))
