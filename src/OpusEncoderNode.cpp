...
void OpusEncoderNode::_bind_methods() {
    ClassDB::bind_method(D_METHOD("encode", "rawPcm"), &OpusEncoderNode::encode);
    std::cout << " OpusEncoderNode bind methods run \n";
...
PackedByteArray OpusEncoderNode::encode(const PackedByteArray rawPcm)
{
	...
	while(!done)
	{
		...
		opus::ensure_buffer_size(encodedBytes, outPos, 4);
		...
		opus::ensure_buffer_size(encodedBytes, outPos, opusPacketSize);
